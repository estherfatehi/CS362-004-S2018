...
int main() {
	...
	//change player's first card to smithy
	testG.hand[thisPlayer][0] = k[8];
	...
	if (testG.playedCards[0] == smithy) {
		printf("PASS: smithy card is in the played pile\n");
	}
	else {
		printf("FAIL: smithy card is not in the played pile\n");
	}
	
	//test that smithy is no longer in the deck/hand
	for (i = 0; i < testG.handCount[thisPlayer]; i++) {
		if (testG.hand[thisPlayer][i] == 14) {
			printf("FAIL: smithy card is still in hand\n");
			flag = 1;
			break;
		}
	}
	for (i = 0; i < testG.deckCount[thisPlayer]; i++) {
		if (testG.deck[thisPlayer][i] == 14) {
			printf("FAIL: smithy card is still in deck\n");
			flag = 1;
			break;
		}
	}
	if (flag == 0) { printf("PASS: smithy is no longer in either hand or deck\n"); }
