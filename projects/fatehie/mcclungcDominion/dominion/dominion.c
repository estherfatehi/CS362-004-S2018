...
int adventurerEffect(int handPos, int drawntreasure, int currentPlayer, struct gameState *state, int cardDrawn, int z);
...
int villageEffect( int currentPlayer, struct gameState *state, int handPos) {
	...
	discardCard(handPos, currentPlayer, state, 0);
	//printf("numActions: %d\n", state->numActions);    WAS FOR TESTING PURPOSES ONLY
	return 0;
}
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    ...
    case adventurer:
		return adventurerEffect(handPos, drawntreasure, currentPlayer, state, cardDrawn, z);
